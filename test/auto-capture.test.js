...
describe('auto-capture', () => {
  ...
  describe('message classification', () => {
    ...
    it('captures correction patterns', async () => {
      ...
      await hook(
        {
          messages: [{ role: 'user', content: "No, that's wrong! The meeting is on Friday." }]
        },
        {}
      );
    ...
    it('captures fact patterns (price)', async () => {
      ...
      await hook(
        {
          messages: [{ role: 'user', content: 'Die Lizenz kostet jetzt €450 pro Jahr statt €350.' }]
        },
        {}
      );
      ...
      await hook(
        {
          messages: [
            {
              role: 'user',
              content: 'Wichtig: nie wieder ohne Backup deployen, das war ein Fehler.'
            }
          ]
        },
        {}
  ...
  describe('return value', () => {
    it('always returns undefined (auto-capture never injects context)', async () => {
      ...
      const result = await hook(
        {
          messages: [
            { role: 'user', content: 'Wichtig: Das ist ein Fehler den wir nie wiederholen.' }
          ]
        },
        {}
      );
