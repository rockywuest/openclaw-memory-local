  ...
  describe("qdrant-client", () => {
    ...
    describe("searchMemories()", () => {
      ...
      it("truncates query to 200 chars", async () => {
        ...
        const storeCall = mockExecFileCalls.find(c =>
          c.args.some(a => a.includes("qdrant-find"))
        );
    ...
    describe("storeMemory()", () => {
      it("stores text via mcporter", async () => {
        ...
        assert.equal(result, true);
        const storeCall = mockExecFileCalls.find(c =>
          c.args.some(a => a.includes("qdrant-store"))
        );
  ...
  describe("auto-recall", () => {
    describe("extractKeywords", () => {
      ...
      it("filters stop words from search query", async () => {
        ...
        await hook({
          messages: [
            { role: "user", content: "Was ist der Deploy-Zeitplan für das Dashboard?" },
          ],
        }, {});

        // Check that the qdrant-find call doesn't contain stop words
        const findCall = mockExecFileCalls.find(c =>
          c.args.some(a => a.includes("qdrant-find"))
        );
        if (findCall) {
